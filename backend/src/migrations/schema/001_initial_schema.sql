...
-- Key column comments
COMMENT ON COLUMN product_configurations.extras_other IS 'JSONB array of custom extra items: [{title: string, cost: number}]';
COMMENT ON COLUMN quote_requests.quote_number IS 'Unique quote identifier in format Q[1-4]-YYYY-NNNNN';
COMMENT ON COLUMN quote_requests.retention_expires_at IS 'Data retention expiry date for GDPR compliance';
...
BEGIN
    ...
    IF table_count != 5 THEN
        RAISE EXCEPTION 'Schema migration failed: Expected 5 tables, found %', table_count;
    END IF;
